        ...
        ambientRecognition.onresult = (event) => {
          ...
          if (currentTranscript.trim()) {
            ...
            if ((hasInterviewKeyword || isQuestionFormat) && currentTranscript.trim().length > 10) {
              ...
              // IMMEDIATE AUTO-SUBMISSION for live mode
              if (autoSubmitRef.current && liveModeRef.current) { // Always true in auto mode

                console.log('🎯 LIVE MODE AUTO-SUBMISSION TRIGGERED');
                console.log('📝 Question to submit:', currentTranscript.trim());

                // For final transcripts - submit immediately with short delay
                if (finalTranscript && finalTranscript.trim().length > 5) {
                  console.log('🚀 FINAL transcript - submitting now!');
                  ...
                  // Submit after very short delay to ensure completeness
                  debounceTimeoutRef.current = setTimeout(() => {
                    console.log('✅ SUBMITTING QUESTION NOW:', finalTranscript.trim());
                ...
                } else if (interimTranscript && currentTranscript.trim().length > 15) {
                  // For long interim transcripts - wait a bit longer
                  console.log('🟡 Long interim transcript - preparing to submit');
                  ...
                  // Submit interim if it seems complete
                  debounceTimeoutRef.current = setTimeout(() => {
                    if (liveModeRef.current && !loadingRef.current) {
                      console.log('✅ SUBMITTING INTERIM QUESTION:', currentTranscript.trim());
